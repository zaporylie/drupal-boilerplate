...
if [ ! -f /project/drupal/sites/default/settings.php ]; then
  # Install vanilla Drupal
  cd /project/drupal/
  drush site-install standard -y --account-name=admin --account-pass=admin --db-url="mysqli://drupal:drupal@localhost/drupal" --db-su=root --db-su-pw=drupal
fi
